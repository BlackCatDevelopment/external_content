...
if (!class_exists('vimeo', false))
{
    final class vimeo extends \CAT\Base implements \CAT\Addon\external_content\IManager
    {
        private static $tablename = 'mod_external_content_vimeo';
        ...
        public function getdata(array $section) : array
        {
            ...
            if(is_array($data)) {
                if(isset($data['link']) && !\CAT\Backend::isBackend()) {
        ...
        protected function checkTable($section_id)
        {
            if(!self::db()->tableExists(self::$tablename)) {
                self::db()->query(
                    ...
                    'INDEX `FK_vim_:prefix:mod_extcont_:prefix:sections` (`section_id`), ' .
                    'CONSTRAINT `FK_vim_:prefix:mod_extcont_:prefix:sections` FOREIGN KEY (`section_id`) REFERENCES `:prefix:sections` (`section_id`) ON UPDATE CASCADE ON DELETE CASCADE ' .
